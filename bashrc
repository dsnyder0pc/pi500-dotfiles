...
# Add custom ~/bin directory to PATH if it exists (Idempotent check)
if [ -d "$HOME/bin" ] ; then
  case ":$PATH:" in
    *:"$HOME/bin":*)
...
# --- PYENV and VIRTUALENV INITIALIZATION (Idempotent) ---
if [ -z "$__PYENV_LOADED" ]; then
  # Set PYENV_ROOT
  export PYENV_ROOT="$HOME/.pyenv"

  # Conditionally add $PYENV_ROOT/bin to PATH (and check if already present)
  ...
  # Initialize pyenv and virtualenv (scripts handle their own PATH updates)
  if command -v pyenv 1>/dev/null 2>&1; then
    eval "$(pyenv init - bash)"
    eval "$(pyenv virtualenv-init -)"
	fi

  # Set sentinel variable to prevent repeated loading
...
if command -v keychain &>/dev/null; then
  ...
  if [ ${#KEYS_TO_LOAD[@]} -gt 0 ]; then
    ...
    eval "$(keychain --eval "${KEYS_TO_LOAD[@]}")"
  else
